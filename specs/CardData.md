...
- **OQ-CD-4** **Fuente del corpus** (`OQ-1`): **copyright resuelto** — el corpus es un **reskin original** (nota legal de `CLAUDE.md`, `OQ-1`): cartas, valores, nombres, arte y tema **propios**; **no** se distribuye contenido de Paleo. El esquema (`CD-*`) deriva de entender el sistema, no de copiar contenido. **Sigue abierto** lo puramente técnico: formato del dataset (JSON/YAML), ubicación (¿`shared/`?, ¿`backend/`?) y el **nombre/tema** concretos del reskin (a fijar al montar el corpus). No bloquea el esquema ni el motor.
