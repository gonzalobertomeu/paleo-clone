...
- **OQ-API-5** [RESUELTA → `BE-12.2`, `OQ-BE-2`] **Formato de `stateChanged`** (`API-12.2`): decidido — **snapshot completo** por cambio. La vista es autosuficiente y reconstruible (`ARCH-9.6`, `API-12.8`) y `GameState` es pequeño (`BE-6`); los *diffs* se descartan en v1 por complejidad. Reevaluable como optimización si el corpus hace crecer la vista (`OQ-BE-2`).
- **OQ-API-6** [RESUELTA → `BE-8`] **Granularidad de `day.resolveStep`** (`API-8.4`): decidido — **paso a paso, dirigido por servidor**: hay como mucho un `pendingStep` por resolución activa, el servidor lo publica en la vista (`API-4.5`), el jugador responsable lo responde con `resolveStep` y el servidor computa el siguiente. Máxima validabilidad y soporte de resolución parcial (`API-8.5`). El **vocabulario** exacto de pasos se cierra contra el corpus A/B (`OQ-BE-1`).
