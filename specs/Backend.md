...
> **Estado:** v0.2 — Motor de reglas y máquina de estados día/noche que ejecuta el protocolo. Módulos, puertos, reductor puro, resolución paso a paso, azar sembrado, persistencia y proyección por jugador. Backend organizado en `src/modules/*` (negocio) y `src/libs/*` (técnico) (`BE-2.6`, `BE-2.7`, `ARCH-5.8`–`ARCH-5.11`). Cierra `OQ-API-5` (formato de `stateChanged`) y `OQ-API-6` (granularidad de `resolveStep`).
...
- **BE-2.5** La **inyección de dependencias de NestJS vive en el anillo técnico externo** —la `infrastructure` de los módulos y los `libs/*`— **nunca** en `domain` ni `application` (`ARCH-4.4`, `ARCH-5.10`). `domain` y `application` no importan NestJS.
- **BE-2.6** **Estructura de `src/`** (`ARCH-5.8`–`ARCH-5.11`): el backend se organiza en **`src/modules/*`** —los tres módulos de negocio `room`/`game`/`view`, cada uno con sus capas— y **`src/libs/*`** —bloques técnicos agnósticos: `trpc`, `redis`, `rng`, `config`—. **Ninguna regla del juego vive en `libs/*`** (`ARCH-1.3`). Un adaptador que implementa un puerto del dominio (repositorio, azar) vive en la `infrastructure` de su módulo y **usa** el `lib` correspondiente (`ARCH-5.11`); `libs/* ↛ modules/*`.
- **BE-2.7** **Transporte y `AppService`.** El *plumbing* de tRPC —contexto, adaptador WS y servir el `appRouter` de `shared` (`API-1.5`)— vive en **`libs/trpc`** y **recibe** el `AppService` por inyección; **no lo implementa**. La **implementación del `AppService`** —orquestación de los casos de uso de los módulos— es **lógica de negocio** y vive en `modules/*` (`application`), compuesta en el *composition root* (`main.ts`). `libs/trpc` solo consume la **interfaz** `AppService` (de `shared`) y no conoce los módulos (`ARCH-5.10`). **Mientras no existan casos de uso**, `libs/trpc` provee un **placeholder no-op** del `AppService` (todo `NOT_IMPLEMENTED`), que mantiene los módulos libres de tRPC; la implementación real se moverá a `modules/*/application` al escribir los casos de uso.
...
- **BE-4.1** **`GameRepository`** — persistencia del agregado (`ARCH-9.2`). Carga y guarda el `GameState` completo por identificador de partida. El dominio declara la interfaz; el **adaptador Redis vive en `modules/game/infrastructure`** y **usa el cliente genérico de `libs/redis`** (`ARCH-5.11`, `BE-10`). El dominio no sabe que Redis existe (`ARCH-9.2`).
- **BE-4.2** **`Rng`** — fuente de azar **sembrada y determinista** (`ARCH-4.6`). Es una **función pura** de `(seed, drawIndex) → valor`, no un generador con estado interno oculto. El **estado del stream** (semilla + nº de extracciones consumidas) vive en el `GameState` (`BE-9`), no en el puerto. Así se concilian `ARCH-4.6` (azar inyectado, sustituible en test) y `ARCH-4.5` (dominio puro): se **inyecta el algoritmo**, pero **evaluar el azar es una función pura del estado**. El **algoritmo genérico** (un PRNG puro) vive en **`libs/rng`** —sin conocer el puerto ni el dominio— y un **adaptador en `modules/game/infrastructure`** lo enchufa al puerto `Rng` (`ARCH-5.11`).
...
- **BE-10.5** El adaptador Redis vive en `modules/game/infrastructure` y usa el cliente de `libs/redis` (`ARCH-5.11`); serializa/deserializa el `GameState` plano (`BE-6.8`). Ninguna regla vive en el adaptador (`ARCH-9.2`): solo persistencia.
